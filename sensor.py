...
class SensorData:
    def __init__(self, is_valid, temperature, humidity):
...
data = config.dht11_data_bcm  # DHT11的data引脚连接到的树莓派的GPIO引脚，使用BCM编号
...
def read():
    ...
    if check == check_tmp and temperature != 0 and temperature != 0:  # 判断数据是否正常
        sensor_data = SensorData(True, temperature, humidity)
    else:
        sensor_data = SensorData(False, None, None)
